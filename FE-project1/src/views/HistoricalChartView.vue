...
const selectedCurrentInterval = ref(60);
const selectedWeightInterval = ref(60);
...
const upsertCurrentChart = () => {
  ...
  currentChartInstance.value = new ChartJS(ctx, {
    ...
    data: {
      labels,
      datasets: [{ label: "Current (A)", data, borderWidth: 2, pointRadius: 2, tension: 0.3 }],
    },
    options: {
      responsive: true,
      ...
      plugins: { legend: { display: false }, ...buildZoomOptions() },
    },
  });
};
...
const upsertWeightChart = () => {
  ...
  weightChartInstance.value = new ChartJS(ctx, {
    ...
    data: {
      labels,
      datasets: [{ label: "Steel Ball (KG)", data, borderWidth: 2, pointRadius: 2, tension: 0.2 }],
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
...
/* =========================================================
 * ✅ Range params (IMPORTANT: dùng function declaration để
 * downloadCsv gọi được)
 * ========================================================= */
...
const downloadCsv = () => {
  ...
  // ===== 5) AGG STEEL BALL (bucket + cumulative) =====
  csv += "AGG_STEEL_BALL\n";
  ...
  link.href = url;
  link.setAttribute("download", `historical-report_ALL_${selectedReportType.value}_${from}_to_${to}.csv`);
...
/* =========================================================
 * ✅ Fetch Historical (ANTI-SPAM): Abort previous request
 * ========================================================= */
...
const fetchHistorical = async () => {
  try {
    ...
    // ✅ cancel previous
    if (historicalAbort) historicalAbort.abort();
...
onUnmounted(() => {
  if (historicalAbort) {
    historicalAbort.abort();
    historicalAbort = null;
  }
});
</script>
