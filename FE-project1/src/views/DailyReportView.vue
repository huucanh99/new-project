...
const fetchCarbonSetting = async () => {
  ...
  const type = (steelBallType.value || "").trim();
  if (!type) {
    carbonCoefficient.value = null;
    carbonUnit.value = "kgCO2/kWh";
    return;
  ...
  try {
    ...
    const coeff = s?.carbonCoefficient ?? null;
    const unit = s?.carbonUnit ?? "kgCO2/kWh";
...
/* ====== ✅ AbortController to prevent overlapping daily-report calls ====== */
let dailyReportAbort = null;

/* ====== GỌI API /api/daily-report ====== */
const loadDailyReport = async () => {
  // ✅ do nothing if not logged in or route not active
  if (!auth.isLoggedIn) return;
  // if you have a route name for this view, optionally lock it:
  // if (route.name && route.name !== "DailyReport") return;
  ...
  try {
    ...
    if (dailyReportAbort) dailyReportAbort.abort();
    dailyReportAbort = new AbortController();
    ...
    const res = await apiFetch(`/api/daily-report?${params.toString()}`, {
      signal: dailyReportAbort.signal,
    });
    ...
    // ✅ avoid loops: only set batchId if needed
...
const steelTotal = computed(() => {
  ...
  return steelBatches.value.reduce((sum, item) => sum + (Number(item.value || 0) || 0), 0);
});
...
const powerTotal = computed(() => {
  ...
  return powerBatches.value.reduce((sum, item) => sum + (Number(item.value || 0) || 0), 0);
});
...
const carbonEmissionDisplay = computed(() => {
  const coeff = Number(carbonCoefficient.value);
  if (Number.isNaN(coeff)) return "--";
  return format2(carbonEmissionTotal.value);
});
...
/* ====== ✅ debounce loader (giảm spam API) ====== */
let loadTimer = null;
const loadDailyReportDebounced = () => {
  clearTimeout(loadTimer);
  loadTimer = setTimeout(() => loadDailyReport(), 250);
};

/* ====== ✅ prevent mount-init watchers from double calling ====== */
...
onMounted(() => {
  ...
  if (auth.isLoggedIn) {
    initDone.value = true;
    loadDailyReport(); // single initial call (watchers won't run until initDone true)
  } else {
    initDone.value = true; // allow future watchers after login
  }
});
...
onUnmounted(() => {
  ...
  if (dailyReportAbort) {
    dailyReportAbort.abort();
    dailyReportAbort = null;
  }

  // ✅ cleanup mouse listeners in case user was dragging
  window.removeEventListener("mousemove", onTimeMouseMove);
  window.removeEventListener("mouseup", onTimeMouseUp);
...
/* ✅ stop work on route leave (important in Electron + keep-alive cases) */
onBeforeRouteLeave(() => {
  clearTimeout(loadTimer);
  if (dailyReportAbort) {
    dailyReportAbort.abort();
...
/**
 * ✅ Watch triggers:
 * - Date / hour / report / shift đổi => reload
 * - BatchId chỉ reload khi đang Batch Summary
 * - Guard with auth + initDone so it never runs pre-login
 */
watch([selectedDate, timeHour, selectedReport, selectedShift], () => {
  if (!initDone.value) return;
  if (!auth.isLoggedIn) return;
  loadDailyReportDebounced();
});

watch([selectedBatchId], () => {
  if (!initDone.value) return;
  if (!auth.isLoggedIn) return;
  if (selectedReport.value === "Batch Summary") loadDailyReportDebounced();
});

/* ✅ If user logs out, stop everything immediately */
watch(
  () => auth.isLoggedIn,
  (loggedIn) => {
    if (!loggedIn) {
      ...
      // destroy charts to release memory immediately
      if (powerChartInstance.value) {
        powerChartInstance.value.destroy();
        powerChartInstance.value = null;
      }
...
/* redraw charts (avoid deep watch; only rebuild when data references change) */
watch(
