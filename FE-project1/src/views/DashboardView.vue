...
/* ====== COMPUTED ====== */
const hasActiveAlarm = computed(() => alarmRows.value.some((row) => !row.end));
...
/* =========================================================
 * ✅ ANTI-SPAM / ANTI-LAG
 * - AbortController: huỷ request cũ khi tick mới tới
 * - silent refresh: không set loading mỗi 5s
 * ========================================================= */
let dashboardAbort = null;
let alarmsAbort = null;
...
const fetchDashboard = async ({ silent = false } = {}) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(msg || "Dashboard fetch failed");
    }
...
const fetchAlarms = async ({ silent = false } = {}) => {
  ...
  try {
    // ✅ cancel previous
    if (alarmsAbort) alarmsAbort.abort();
    alarmsAbort = new AbortController();

    const res = await apiFetch("/api/alarms", {
      signal: alarmsAbort.signal,
    });

    if (!res.ok) {
      const msg = await res.text().catch(() => "");
      throw new Error(msg || "Alarm fetch failed");
    }
    alarmRows.value = await res.json();
  } catch (e) {
    if (e?.name === "AbortError") return;
    alarmError.value = e?.message || String(e);
  } finally {
    if (!silent) alarmLoading.value = false;
  }
};
...
const ackAlarm = async (id) => {
  try {
    ...
    // ✅ refresh once (silent to avoid UI flicker)
    await Promise.all([fetchAlarms({ silent: true }), fetchDashboard({ silent: true })]);
  } catch (e) {
...
const openAlarmModal = async () => {
  await fetchAlarms(); // not silent because opening modal should show loading state
...
async function tick() {
  ...
  // ✅ silent refresh to reduce rerender/flicker
  await Promise.all([
    fetchDashboard({ silent: true }),
    fetchAlarms({ silent: true }),
  ]);
}
...
function startPolling() {
  ...
  // ✅ first load (show loading only once)
  fetchDashboard({ silent: false });
  fetchAlarms({ silent: false });

  timer = setInterval(tick, POLL_MS);
}

/* ✅ Resume polling when user returns to tab/app */
function handleVisibilityChange() {
  if (!auth.isLoggedIn) return;
  if (!document.hidden) {
    // run one tick immediately when back
    tick();
  }
}
...
/* ✅ Important for Electron + keep-alive / route transitions */
onBeforeRouteLeave(() => {
  stopPolling();
});
</script>
