...
/**
 * Persists bypass toggle state to localStorage.
 */
...
/**
 * Loads the set of component IDs that have already shown a warning alert.
 */
...
/**
 * Saves the set of alerted component IDs to localStorage.
 */
...
/**
 * Formats elapsed seconds as hours with 2 decimals.
 */
const formatHours = (seconds) => getHours(seconds).toFixed(2);

/**
 * Determines whether a component is in alarm state based on accumulated vs warning hours.
 */
...
/**
 * Fetches component life data from the backend and updates the UI state.
 */
...
/**
 * Starts polling the backend to refresh UI values periodically.
 */
...
/**
 * Saves warning hours for a single component.
 */
...
/**
 * Resets accumulated hours for a component and re-enables future alerts for it.
 */
const resetComponent = async (item) => {
  const ok = confirm(
    t("lifeWarning.resetConfirm") || `Reset ${t(item.nameKey)} accumulated hours to 0?`
  );
  if (!ok) return;
...
/**
 * Navigates back to the general settings page.
 */
