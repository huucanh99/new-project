...
/**
 * Persists the selected language to localStorage whenever it changes.
 */
...
export const translations = {
  en: {
    ...
    lifeWarning: {
      ...
      lifeWarningTriggered: "{{name}} has reached its warning limit ({{hours}} hours).",
      saving: "Saving...",
      resetting: "Resetting...",
      saveSuccess: "Saved successfully.",
      savedOne: "{{name}} warning hours have been saved.",
      resetConfirm: "Reset {{name}} accumulated hours to 0 and start counting again?",
  ...
  zh: {
    ...
    lifeWarning: {
      ...
      lifeWarningTriggered: "{{name}} 的累積運轉時間已達警報時間（{{hours}} 小時）。",
      saving: "儲存中…",
      resetting: "重置中…",
      saveSuccess: "已成功儲存。",
...
/**
 * Small i18n hook for Vue components.
 * - Supports dot-keys: "dailyReport.steelBall"
 * - Supports interpolation: t("lifeWarning.lifeWarningTriggered", { name: "...", hours: "2.00" })
 */
export const useI18n = () => {
  /**
   * Translates a dot-separated key using the current language pack.
   */
  const t = (key, vars) => {
  ...
  /**
   * Convenience helper for status labels: ts("operating") -> t("status.operating")
   */
